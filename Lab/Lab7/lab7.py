...
class User:
    def __init__(self, citizen_id, name):
        if(len(citizen_id.replace("-", "")) != 13): return "invalid id"
...
class Account:
    ...
    def __add__(self, amount):
        # print("\n\n\n\nBefore deposite : ", self.amount)
        self.__amount += amount
        # print("After deposite : ", self.amount, "\n\n\n")
        self.__transaction.append(Transaction("D", amount, self.__amount, self))
    
    def __sub__(self, amount):
        # print("\n\n\n\nBefore withdraw : ", self.amount)
        self.__amount -= amount
        # print("After withdraw : ", self.amount, "\n\n\n")
        self.__transaction.append(Transaction("W", amount, self.__amount, self))
    
    def __rshift__(self, data_tuple):
        account, amount = data_tuple
        self.__amount -= amount
        # print("\n\n\n\nBefore transfer : ", account.amount)
        account + amount
        # print("After transfer : ", account.amount, "\n\n\n")
        self.update_transaction(Transaction("T-", amount, self.__amount, self))
        account.update_transaction(Transaction("T+", amount, account.amount, self))
...
class Transaction:
    ...
    def __str__(self):
        return f"{self.__transaction_type} {self.__amount}, Balance: {self.__total} From: {self.__target_account.account_no}"
...
class ATM_machine:
    ...
    def deposit(self, account, amount):
        if(isinstance(account, Account) and amount >= 0):
            account + amount

    def withdraw(self, account, amount):
        if(account.amount > amount and amount >= 0 and amount <= ATM_machine.withdraw_limit):
            account - amount
        else:
            return "Not Enough credit"

    def transfer(self, account, amount, target_account):
        # target_account >> (account, amount)
...
class EDC_machine:
    ...
    def paid(self, card, amount, account):
...
# scb.atm_list[0].deposit(harry_account, 1000)
# scb.atm_list[0].transfer(harry_account, 1000, hermione_account1)
